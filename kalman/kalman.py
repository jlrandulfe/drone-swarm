...
class Kalman:
	...
	def predict(self, relative_velocities):
		print(" PREDICTION ")
		print("\n")
	...
	def update(self, distance_sensor):
		print(" UPDATE ")
		print("\n")
		# correct the KF
		...
		dist = (self.distance(previous_state[0], previous_state[1]))
		print(" dist: ", dist)
		print("\n")
		...
		if dist.any() == 0:
			...
			self.H = (1 / dist) * previous_state.transpose()
		...
		self.state_X = self.state_X + self.K * (self.error(previous_state, distance_sensor)) # distance_measure should be an input!!
		#self.cov_P = self.cov_P - self.K.dot(self.H.dot(self.cov_P))
		...
		if not np.all(la.eigvals(self.cov_P) > 0):
			self.state_X = previous_state
			self.cov_P = previous_cov

	def error(self, previous_state, distance_sensor):

		# for tracking the error: here we compare the distance that we obtain with the sensor vs the distance calculated
		# by the position estimation.
		dist_estimation = self.distance(previous_state[0], previous_state[1])
